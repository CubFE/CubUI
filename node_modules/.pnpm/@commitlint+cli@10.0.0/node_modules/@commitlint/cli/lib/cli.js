...
async function main(options) {
    ...
    const loaded = await load_1.default(getSeed(flags), loadOpts);
    ...
    const results = await Promise.all(messages.map((message) => lint_1.default(message, loaded.rules, opts)));
...
function loadFormatter(config, flags) {
    ...
    if (modulePath) {
        ...
        if (isFunction_1.default(moduleInstance.default)) {
