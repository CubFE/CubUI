...
fs.outputFile(path.resolve(__dirname, '../src/packages/cubui.vue.build.ts'), fileStrBuild, 'utf8', (error) => {
...
fs.outputFile(path.resolve(__dirname, '../src/packages/cubui.vue.ts'), fileStrDev, 'utf8', (error) => {
  // logger.success(`${package_config_path} 文件写入成功`);
});
