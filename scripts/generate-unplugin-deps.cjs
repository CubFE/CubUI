...
if (!target) {
  console.error('缺少 cubui or cubui-taro 参数！');
  return;
}
...
config.nav.forEach((item) => {
  item.packages.forEach((element) => {
    ...
    if (target === 'cubui-taro' && element.taro === false) return; // 排除无 Taro 版组件
    // gen entry
    if (element.exclude != true) {
      let outputMjs = '';
      if (element.type == 'methods' && target == 'cubui') {
...
tasks.push(
  fs.outputFile(path.resolve(__dirname, `../dist/cubui.es.js`), outputFileEntry, 'utf8', () => {
    // console.log('')
  })
);
