<template>
  <cub-tabs class="cub-cascader" v-model="tabsCursor" @click="handleTabClick" title-scroll>
    <template v-if="!initLoading && panes.length">
      <cub-tab-pane v-for="(pane, index) in panes" :title="formatTabTitle(pane)" :key="index">
        <view role="menu" class="cub-cascader-pane">
          <cub-scroll-view :scrollY="true" style="height: 100%">
            <template v-for="node in pane.nodes" :key="node.value">
              <view
                class="cub-cascader-item"
              ...
              >
                <view class="cub-cascader-item__title">{{ node.text }}</view>

                <Loading v-if="node.loading" class="cub-cascader-item__icon-loading" name="loading" />
                <Checklist v-else class="cub-cascader-item__icon-check" name="checklist" />
              </view>
            </template>
          </cub-scroll-view>
        </view>
    ...
    <template v-else>
      <cub-tab-pane title="Loading...">
        <view class="cub-cascader-pane"></view>
      </cub-tab-pane>
    </template>
  </cub-tabs>
</template>
...
import { Loading, Checklist } from '@cubfe/icons-vue-taro';
