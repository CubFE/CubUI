<template>
  <view :class="classes">
    <view class="cub-elevator__list" ref="listview" :style="{ height: isNaN(+height) ? height : `${height}px` }">
      <view class="cub-elevator__list__item" v-for="item in indexList" :key="item[acceptKey]" :ref="setListGroup">
        <view class="cub-elevator__list__item__code">{{ item[acceptKey] }}</view>
        <view
          class="cub-elevator__list__item__name"
          :class="{
            'cub-elevator__list__item__name--highcolor': currentData.id === subitem.id && currentKey === item[acceptKey]
          }"
      ...
      <view class="cub-elevator__list__fixed" :style="fixedStyle" v-show="scrollY > 0" v-if="isSticky">
        <span class="cub-elevator__fixed-title">{{ indexList[currentIndex][acceptKey] }}</span>
      </view>
    </view>
    <view class="cub-elevator__code--current" v-show="scrollStart" v-if="indexList.length">{{
      indexList[codeIndex][acceptKey]
    }}</view>
    <view class="cub-elevator__bars" @touchstart="touchStart" @touchmove.stop.prevent="touchMove" @touchend="touchEnd">
      <view class="cub-elevator__bars__inner">
        <view
          class="cub-elevator__bars__inner__item"
