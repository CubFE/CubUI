<template>
  <view>
    <view :class="classes" @click="handleClick" ref="root" :id="'root' + refRandomId">
      <view class="cub-ellipsis__wordbreak" v-if="!exceeded">{{ content }}</view>

      <view v-if="exceeded && !expanded" class="cub-ellipsis__wordbreak">
        {{ ellipsis.leading }}{{ ellipsis.leading && symbol
        }}<view class="cub-ellipsis__text" v-if="expandText" @click.stop="clickHandle(1)">{{ expandText }}</view
        >{{ ellipsis.tailing && symbol }}{{ ellipsis.tailing }}
      </view>
      <view v-if="exceeded && expanded">
        {{ content }}
        <span class="cub-ellipsis__text" v-if="expandText" @click.stop="clickHandle(2)">{{ collapseText }}</span>
      </view>
    </view>

    <view class="cub-ellipsis__copy" ref="rootContain" :id="'rootContain' + refRandomId" :style="{ width: widthRef }">
      <view>{{ contantCopy }}</view>
    </view>

    <!-- 省略号 symbol  -->
    <view class="cub-ellipsis__copy" ref="symbolContain" :id="'symbolContain' + refRandomId" style="display: inline">{{
      symbolText
    }}</view>
