<template>
  <cub-popup pop-class="cub-image-preview-custom-pop" v-model:visible="showPop">
    <view class="cub-image-preview" @touchstart.capture="onTouchStart">
      <cub-swiper
        v-if="showPop"
        :auto-play="autoplay"
        class="cub-image-preview-swiper"
        :loop="isLoop"
      ...
      >
        <cub-swiper-item v-for="(item, index) in images" :key="index">
          <img :src="item.src" mode="aspectFit" class="cub-image-preview-img" @click.stop="closeOnImg" />
        </cub-swiper-item>
      </cub-swiper>
    </view>

    <view class="cub-image-preview-index" v-if="showIndex"> {{ active + 1 }} / {{ images.length }} </view>
    <view class="cub-image-preview-close-icon" @click="onClose" :style="styles" v-if="closeable"
...
import { CircleClose } from '@cubfe/icons-vue-taro';
...
export default create({
  ...
  setup(props, { emit }) {
    ...
    const init = () => {
      state.eleImg = document.querySelector('.cub-image-preview');
