<template>
  <view :class="classes">
    <view :id="'cub-menu__bar' + refRandomId" class="cub-menu__bar" :class="{ opened: opened }" ref="barRef">
      <template v-for="(item, index) in children" :key="index">
        <view
          class="cub-menu__item"
        ...
        >
          <view class="cub-menu__title" :class="getClasses(item.state.showPopup)">
            <view class="cub-menu__title-text">{{ item.renderTitle() }}</view>
            <span class="cub-menu__title-icon">
              <slot name="icon">
...
import { RectUp, RectDown } from '@cubfe/icons-vue-taro';
