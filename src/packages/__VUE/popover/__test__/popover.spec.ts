...
test('first render', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-popover-menu-item').exists()).toBeTruthy();
});
...
test('Props theme dark', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-popover--dark').exists()).toBeTruthy();
});
...
test('should not emit select event when the action is disabled', async () => {
  ...
  await nextTick();
  expect(wrapper.findAll('.cub-popover-menu-disabled').length).toEqual(2);

  wrapper.find('.cub-popover-menu-item').trigger('click');
...
test('should close popover when clicking the action', async () => {
  ...
  await wrapper.find('.cub-popover-menu-item').trigger('click');
...
test('Set Props Position', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-popover-arrow--top-start').exists()).toBeTruthy();
});
