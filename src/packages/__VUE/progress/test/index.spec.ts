...
test('should render different height and color when use color height props', async () => {
  ...
  await nextTick();
  const inner = wrapper.find<HTMLElement>('.cub-progress-inner');
  expect(inner.element.style.background).toEqual('blue');
  const outer = wrapper.find<HTMLElement>('.cub-progress-outer');
  const span = wrapper.find<HTMLElement>('.cub-progress-text span');
...
test('should hide percentage when use showText props', () => {
  ...
  const text = wrapper.find<HTMLElement>('.cub-progress-text');
...
test('should render inside percentage when use textInside props', () => {
  ...
  const text = wrapper.find<HTMLElement>('.cub-progress-insidetext');
...
test('should render custom size when use size props', () => {
  ...
  const text = wrapper.find<HTMLElement>('.cub-progress-large');
  expect(text.exists()).toBe(true);
});
