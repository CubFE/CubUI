...
test('base rate', () => {
  ...
  const rate = wrapper.find('.cub-rate');
  expect(rate.exists()).toBe(true);
});

test('should have rate when v-model', () => {
  const wrapper = mount(Rate, { props: { modelValue: 4 } });
  const rate = wrapper.findAll('.cub-rate-item__icon--disabled');
...
test('should have the same count and activeColor', () => {
  ...
  const rate = wrapper.findAll('.cub-rate-item');
  expect(rate.length).toBe(10);

  const i = wrapper.findAll('.cub-rate-item__icon');
...
test('should have click', async () => {
  ...
  const rate = wrapper.findAll('.cub-rate-item svg');
  rate[2].trigger('click');
  await nextTick();
  const rateDis = wrapper.findAll('.cub-rate-item__icon--disabled');
