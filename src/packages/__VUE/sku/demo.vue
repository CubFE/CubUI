<template>
  <div class="demo">
    ...
    <cub-cell :title="translate('basic')" desc="" @click="base = true"></cub-cell>

    <h2>{{ translate('noSell') }}</h2>
    <cub-cell :title="translate('noSell')" desc="" @click="notSell = true"></cub-cell>

    <h2>{{ translate('customStepper') }}</h2>
    <cub-cell :title="translate('customStepper')" desc="" @click="customStepper = true"></cub-cell>

    <h2>{{ translate('slots') }}</h2>
    <cub-cell :title="translate('slots')" desc="" @click="customBySlot = true"></cub-cell>
    ...
    >
      <template #sku-header-price>
        <div>
          <cub-price :price="goodsInfo.price" :needSymbol="true" :thousands="false"> </cub-price>
          <span class="tag"></span>
        </div>
      </template>

      <template #sku-header-extra>
        <span class="cub-sku-header-right-extra">重量：0.1kg 编号：{{ goodsInfo.skuId }} </span>
      </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getData = () => {
      fetch('//storage.360buyimg.com/cubui/3x/data.js')
