...
import { JoySmile, Dongdong } from '@cubfe/icons-vue';
...
test('base Tabs', () => {
  ...
  const rate = wrapper.find('.cub-tabs');
...
test('base tabs props', async () => {
  const wrapper = mount(Tabs, {
    ...
    components: {
      'cub-tabs': Tabs,
      'cub-tab-pane': TabPane
    }
  });
  await nextTick();
  const stepItem = wrapper.find('.cub-tabs__titles');
  ...
  const _stepItem1 = wrapper.findAll('.cub-tabs__titles')[0];
  expect(_stepItem1.classes()).toContain('smile');
  const _stepItem2 = wrapper.findAll('.cub-tabs__titles')[0];
  expect(_stepItem2.classes()).toContain('large');
  const _stepItem3 = wrapper.findAll('.cub-tabs__titles')[0];
...
test('base other props', async () => {
  const wrapper = mount({
    ...
    template: `
      <cub-tabs animatedTime="500" titleGutter="20">
        <cub-tab-pane paneKey="1">123</cub-tab-pane>
        <cub-tab-pane paneKey="2">456</cub-tab-pane>
      </cub-tabs>
    `
  });
  await nextTick();
  const stepItem = wrapper.find('.cub-tabs__content');
  expect((stepItem.element as HTMLElement).style.transitionDuration).toEqual('500ms');
  const stepItem1 = wrapper.find('.cub-tabs__titles-item');
...
test('base Tabs Slots', async () => {
  const wrapper = mount({
    ...
    <cub-tabs v-model="state.tab7value">
      ...
      <cub-tab-pane v-for="item in state.list6" :pane-key="item.paneKey">
        {{ item.title }}
      </cub-tab-pane>
    </cub-tabs>
    `,
  ...
  await nextTick();
  const tab1 = wrapper.find('.cub-tabs__titles');
  expect(tab1.exists()).toBe(true);
  const tab2 = wrapper.findAll('.cub-tabs__titles-item');
  expect(tab2.length).toBe(3);
  const tab3 = wrapper.findAll('.cub-tabs__titles-item__text');
  expect(tab3[0].html()).toContain('自定义 1');
  expect(tab3[1].html()).toContain('自定义 2');
  expect(tab3[2].html()).toContain('自定义 3');
  const tab4 = wrapper.find('.cub-tabs__content');
...
test('base Tabpane Props', async () => {
  const wrapper = mount({
    ...
    <cub-tabs v-model="state.tab2value">
      <cub-tab-pane title="Tab 1" pane-key="0"> </cub-tab-pane>
  ...
  await nextTick();
  const tab = wrapper.findAll('.cub-tabs__titles-item');
  expect(tab.length).toBe(3);
  const tab1 = wrapper.findAll('.cub-tabs__titles-item')[1];
  expect(tab1.classes()).toContain('disabled');
  const tab2 = wrapper.findAll('.cub-tabs__titles-item')[0];
...
test('base click', async () => {
  ...
  await nextTick();
  const tab = wrapper.find('.cub-tabs__titles-item');
  expect(tab.classes()).toContain('active');
  tab.trigger('click');
  const tab1 = wrapper.find('.cub-tabs__content');
...
test('Tabs: direction=vertical & title-gutter', async () => {
  const wrapper = mount({
    ...
    template: `
    <cub-tabs direction="vertical" title-gutter="10">
      <cub-tab-pane pane-key="1" title="Tab 1"> Tab 1 </cub-tab-pane>
      <cub-tab-pane pane-key="2" title="Tab 2"> Tab 2 </cub-tab-pane>
      <cub-tab-pane pane-key="3" title="Tab 3"> Tab 3 </cub-tab-pane>
    </cub-tabs>
    `,
    setup() {
      const state = reactive({
