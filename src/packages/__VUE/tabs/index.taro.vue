<template>
  <view class="cub-tabs" :class="[direction]" ref="container">
    <cub-scroll-view
      :scroll-x="getScrollX"
      :scroll-y="getScrollY"
      ...
      :enable-flex="true"
      :id="`cub-tabs__titles_${name}`"
      class="cub-tabs__titles tabs-scrollview"
      :class="{ [type]: type, scrollable: titleScroll, 'scroll-vertical': getScrollY, [size]: size }"
      :style="tabsNavStyle"
    >
      <view class="cub-tabs__list">
        <slot v-if="$slots.titles" name="titles"></slot>
        <template v-else>
          <view
            class="cub-tabs__titles-item taro"
            :style="titleStyle"
            ...
            :key="item.paneKey"
          >
            <view class="cub-tabs__titles-item__line" :style="tabsActiveStyle" v-if="type == 'line'"></view>
            <view class="cub-tabs__titles-item__smile" :style="tabsActiveStyle" v-if="type == 'smile'">
              <JoySmile :color="color" />
            </view>
            <view class="cub-tabs__titles-item__text" :class="{ ellipsis: ellipsis }">{{ item.title }} </view>
          </view>
          <view v-if="canShowLabel" class="cub-tabs__titles-item cub-tabs__titles-placeholder"></view>
        </template>
      </view>
    </cub-scroll-view>
    <view
      class="cub-tabs__content"
      ref="tabsContentRef"
...
import { JoySmile } from '@cubfe/icons-vue-taro';
  ...
  setup(props: any, { emit, slots }: any) {
    ...
    const findTabsIndex = (value: string | number) => {
      ...
      if (titles.value.length == 0) {
        // console.warn('[cubui] <Tabs> 当前未找到 TabPane 组件元素 , 请检查 .');
      } else if (index == -1) {
        // console.warn('[cubui] <Tabs> 请检查 v-model 值是否为 paneKey ,如 paneKey 未设置，请采用下标控制 .');
      } else {
    ...
    const scrollIntoView = () => {
      ...
      raf(() => {
        Promise.all([
          getRect(`#cub-tabs__titles_${props.name}`),
          getAllRect(`#cub-tabs__titles_${props.name} .cub-tabs__titles-item`)
