...
test('props v-model', async () => {
  const component = {
    template: `<cub-textarea v-model="value" />`,
    setup() {
      const value = ref('618');
      return {
        value
  ...
  await nextTick();
  const txtWrapper = wrapper.find('.cub-textarea__textarea').element as HTMLTextAreaElement;
  expect(txtWrapper.value).toBe('618');
});

test('props placeholder	max-length rows limit-show', async () => {
  const component = {
    template: `<cub-textarea v-model="value" :placeholder="placeholder" :max-length="maxLength" :limit-show="limitShow" :rows="rows"/>`,
    setup() {
  ...
  expect(wrapper.find('.cub-textarea__textarea').attributes('rows')).toBe(wrapper.vm.rows);
  expect(wrapper.findAll('.cub-textarea__limit')).toHaveLength(1);
});

test('props autosize text-align autofocus', async () => {
  const component = {
    template: `<cub-textarea v-model="value" :autosize="autosize" :text-align="textAlign" :autofocus="autofocus"/>`,
    setup() {
      const value = ref('123456');
  ...
  expect(wrapper.find('.cub-textarea__textarea').attributes('autofocus')).not.toBeUndefined();
});

test('props readonly disabled', async () => {
  const component = {
    template: `<cub-textarea v-model="value" readonly="readonly" :disabled="disabled"/>`,
    setup() {
      const value = ref('123456');
  ...
  const wrapper = mount(component);
  await nextTick();
  const txtWrapper = wrapper.find('.cub-textarea__textarea');
