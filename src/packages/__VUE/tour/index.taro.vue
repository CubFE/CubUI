<template>
  <view :class="classes">
    <view class="cub-tour-masked" v-if="showTour" @click="handleClickMask"></view>

    <view v-for="(step, i) in steps" :key="i" style="height: 0">
      <view
        class="cub-tour-mask"
        :class="[mask ? (showPopup[i] ? '' : 'cub-tour-mask-hidden') : 'cub-tour-mask-none']"
        :style="maskStyles[i]"
        v-if="showTour"
        :id="`cub-tour-popid${i}${refRandomId}`"
      ></view>
      <cub-popover
        v-model:visible="showPopup[i]"
        :location="step.location || location"
        :targetId="`cub-tour-popid${i}${refRandomId}`"
        :bgColor="bgColor"
        :theme="theme"
      ...
      >
        <template #content>
          <slot>
            <view class="cub-tour-content" v-if="type == 'step'">
              <view class="cub-tour-content-top" v-if="showTitleBar">
                <view @click="close">
                  <Close class="cub-tour-content-top-close" size="10px" />
                </view>
              </view>
              <view class="cub-tour-content-inner">
                {{ step.content }}
              </view>
              <view class="cub-tour-content-bottom">
                <view class="cub-tour-content-bottom-init">{{ active + 1 }}/{{ steps.length }}</view>
                <view class="cub-tour-content-bottom-operate">
                  <slot name="prev-step">
                    <view
                      class="cub-tour-content-bottom-operate-btn"
                  ...
                  <view
                    class="cub-tour-content-bottom-operate-btn active"
                    @click="close"
            ...
            <view class="cub-tour-content cub-tour-content-tile" v-if="type == 'tile'">
              <view class="cub-tour-content-inner">
                {{ step.content }}
              </view>
            </view>
