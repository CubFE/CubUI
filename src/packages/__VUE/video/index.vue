<template>
  <div class="cub-video" ref="videocon">
    <video
      ref="root"
      class="cub-video-player"
      :muted="options.muted"
    ...
    <div class="cub-video-mask" ref="touchMask" v-if="showToolbox && !isDisabled" @click="play"></div>
    <div
      class="cub-video-play-btn"
    ...
    ></div>
    <div
      class="cub-video-controller"
      v-show="showToolbox && !isDisabled"
      :class="{ 'cub-video-controller--show': !state.playing, 'cub-video-controller--hide': state.playing }"
    >
      <div class="cub-video-controller__playbtn" @click="play"></div>
      <div class="cub-video-controller__now">{{ videoSet.displayTime }}</div>
      <div class="cub-video-controller__progress">
        <div class="cub-video-controller__progress-value" ref="progressBar">
          <div class="buffered" :style="{ width: `${videoSet.loaded}%` }"></div>
          <div
            class="cub-video-controller__ball"
            :style="{
          ...
          >
            <div class="cub-video-controller__ball-move"></div>
          </div>
          <div class="cub-video-controller__played" ref="playedBar"></div>
        </div>
      </div>
      <div class="cub-video-controller__total">{{ videoSet.totalTime }}</div>
      <div class="cub-video-controller__volume" @click="handleMuted" :class="{ muted: state.isMuted }"></div>
      <div class="cub-video-controller__full" @click="fullScreen"></div>
    </div>
    <!-- 错误弹窗 -->
    <div class="cub-video-error" v-show="state.isError">
      <p class="cub-video-error-tip">{{ translate('errorTip') }}</p>
      <p class="cub-video-error-retry" @click="retry">{{ translate('clickRetry') }}</p>
    </div>
  </div>
</template>
<script lang="ts">
...
export default create({
  ...
  setup(props, { emit }) {
    ...
    const customerInit = () => {
      ...
      const $progress = root.value!.getElementsByClassName('cub-video-controller__progress-value')[0];
      // 播放器位置
