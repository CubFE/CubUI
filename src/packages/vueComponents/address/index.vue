<template>
  <cub-popup
    ...
    :teleport="teleport"
  >
    <view class="cub-address">
      <view class="cub-address__header">
        <view class="cub-address__header-back" @click="switchModule">
          <slot name="back-icon">
            <Left v-show="type == 'exist' && privateType == 'custom'" size="14px"></Left>
          </slot>
        </view>

        <view class="cub-address__header__title">
          {{
            privateType == 'custom'
        ...
        <view class="cub-address__header-close" @click="handClose('cross')">
      ...
      <!-- 请选择 -->
      <view class="cub-address__custom" v-if="['custom', 'custom2'].includes(privateType)">
        <view class="cub-address__region" ref="tabRegion">
          <view
            :class="['cub-address__region-item', index == tabIndex ? 'active' : '']"
          ...
          <view class="active cub-address__region-item" v-if="tabIndex == selectedRegion.length">
            <view>{{ getTabName(null, selectedRegion.length) }} </view>
          </view>

          <view class="cub-address__region-line" ref="regionLine" :style="{ left: lineDistance + 'px', width: lineWidth +'px' }"></view>
        </view>

        <view class="cub-address__detail" v-if="privateType == 'custom'">
          <ul class="cub-address__detail-list" ref="scrollDom">
            <li
              v-for="(item, index) in regionList"
              :key="index"
              :class="['cub-address__detail-item', selectedRegion[tabIndex]?.id == item.id ? 'active' : '']"
              @click="nextAreaList(item)"
            >
              <div>
                <slot name="icon" v-if="selectedRegion[tabIndex]?.id == item.id">
                  <Check class="cub-address-select-icon" size="13px"></Check> </slot
                >{{ item.name }}
              </div>
            </li>
          </ul>
        ...
        <view class="cub-address__elevator-group" v-else>
          <cub-elevator
            :height="height"
      ...
      <!-- 配送至 -->
      <view class="cub-address__exist" v-else>
        <div class="cub-address__exist-group">
          <ul class="cub-address__exist-group-list">
            <li
              :class="['cub-address__exist-group-item', item.selectedAddress ? 'active' : '']"
              v-for="(item, index) in existAddress"
              :key="index"
              @click="selectedExist(item)"
            >
              <slot name="unselected-icon" v-if="!item.selectedAddress">
                <Location class="cub-address-select-icon" size="13px"></Location>
              </slot>

              <slot name="icon" v-if="item.selectedAddress">
                <Check class="cub-address-select-icon" size="13px"></Check>
              </slot>

              <div class="cub-address__exist-item-info">
                <div class="cub-address__exist-item-info-name" v-if="item.name">{{ item.name }}</div>
                <div class="cub-address__exist-item-info-phone" v-if="item.phone">{{ item.phone }}</div>
                <div class="cub-address__exist-item-info-bottom">
                  <view>
        ...
        <div class="cub-address__exist-choose" @click="switchModule" v-if="isShowCustomAddress">
          <div class="cub-address__exist-choose-btn">{{
...
import { Location, Check, Close, Left } from '@cubfe/icons-vue';
  ...
  setup(props: any, { emit }) {
    ...
    const transformData = (data: RegionData[]) => {
      ...
      data.forEach((item: RegionData) => {
        if (!item.title) {
          console.warn('[cubui] <Address> 请检查数组选项的 title 值是否有设置 ,title 为必填项 .');
          return;
        }
      });
    ...
    const lineAnimation = () => {
      ...
      nextTick(() => {
        ...
        if (name) {
          ...
          // lineDistance.value = (distance ? distance : 20) + width/2 - 13;
