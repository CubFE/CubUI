...
test('Animate: should change classname when using type prop', () => {
  for (let i = 0; i < testType.length; i++) {
    ...
    const animate: any = wrapper.find('.cub-animate__container');
    expect(animate.classes(`cub-animate-${typeProp}`)).toBe(true);
  }
});
...
test('Animate: trigger animate', async () => {
  ...
  const animate: any = wrapper.find('.cub-animate__container');
  expect(animate.classes(`cub-animate-${typeProp}`)).toBe(false);
