<template>
  ...
  <div class="cub-audio">
    <!-- 进度条 -->
    <div class="cub-audio__progress" v-if="type == 'progress'">
      <!-- 时间显示 -->
      <div class="cub-audio__time">{{ currentDuration }}</div>
      <div class="cub-audio__bar">
        <cub-range
          v-model="percent"
          hidden-range
        ...
        >
          <template #button>
            <div class="cub-audio__button--custom"></div>
          </template>
        </cub-range>
      </div>

      <div class="cub-audio__time">{{ duration }}</div>
    </div>

    <!-- 自定义 -->
    <div class="cub-audio__icon" v-if="type == 'icon'">
      <div
        :class="['cub-audio__icon--box', playing ? 'cub-audio__icon--play' : 'cub-audio__icon--stop']"
        @click="changeStatus"
      >
        <Service v-if="playing" class="cub-icon-am-rotate cub-icon-am-infinite"></Service>
