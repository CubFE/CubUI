<template>
  <view :class="classes">
    <div ref="dmContainer">
      <div :class="['cub-barrage__slotBody' + classTime]">
        <view
          v-for="(item, index) of danmuList"
          :key="'danmu' + index"
          :class="['cub-barrage__item', 'cub-barrage__item' + index, 'move']"
...
export default create({
  ...
  setup(props) {
    ...
    const classes = computed(() => {
      ...
      return {
        [prefixCls]: true,
        ['cub-barrage--dmBody' + timeId.value]: true
      };
    });
    ...
    onMounted(() => {
      if (slotDefault) {
        const list = document
          .getElementsByClassName('cub-barrage__slotBody' + classTime)[0]
          .getElementsByClassName('cub-barrage__item');
    ...
    const getNode = (index: number) => {
      ...
      setTimeout(() => {
        let width = 100;
        query.select('.cub-barrage--dmBody' + timeId.value).boundingClientRect((rec: any) => {
          width = rec?.width || 300;
        });
        query
          .select('.cub-barrage__item' + index)
