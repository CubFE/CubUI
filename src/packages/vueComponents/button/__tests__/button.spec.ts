...
import { StarFill } from '@cubfe/icons-vue';
...
test('should find cub-icon class of an svg tag when using icon slot', () => {
  const wrapper = mount(Button, {
    slots: {
      icon: StarFill
    }
  ...
  expect(icons[0].element.classList).toContain(['cub-icon']);
});
