...
test('show-title prop', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-calendar__header-title').exists()).toBeTruthy();
  await wrapper.setProps({ showTitle: false });
  expect(wrapper.find('.cub-calendar__header-title').exists()).toBeFalsy();
});
test('show-sub-title prop', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-calendar__header-subtitle').exists()).toBeTruthy();
  await wrapper.setProps({ showSubTitle: false });
  expect(wrapper.find('.cub-calendar__header-subtitle').exists()).toBeFalsy();
});
test('show-today prop', async () => {
  ...
  expect(wrapper.find('.cub-calendar__day-tips--curr').exists()).toBeFalsy();
});
...
test('first-day-of-week prop', async () => {
  ...
  // 头部周第1个元素
  const weekday = wrapper.findAll('.cub-calendar__weekday')[0].text();
  expect(weekday).toEqual(weekdays[2]);
  // 日期面板第7个元素, 判断最后一天的日期是否匹配
  const calendarMonth = wrapper.find('.cub-calendar__month');
  const monthTitle = calendarMonth.find('.cub-calendar__month-title').text();
  const dayText = calendarMonth.findAll('.cub-calendar__day-value')[6].text();
...
test('should render slot correctly', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-calendar__header').html()).toMatchSnapshot();
  expect(wrapper.find('.cub-calendar__body').html()).toMatchSnapshot();
});
...
test('select event when click item', async () => {
  ...
  await nextTick();
  wrapper.findAll('.cub-calendar__day')[15].trigger('click');
