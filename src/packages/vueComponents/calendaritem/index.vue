<template>
  <view
    class="cub-calendar"
    :class="{
      'cub-calendar--nopop': !poppable,
      'cub-calendar--nofooter': isAutoBackFill
    }"
  >
    <!-- header -->
    <view class="cub-calendar__header">
      <view class="cub-calendar__header-title" v-if="showTitle">{{ title || translate('title') }}</view>
      <view class="cub-calendar__header-slot" v-if="showTopBtn">
        <slot name="btn"> </slot>
      </view>
      <view class="cub-calendar__header-subtitle" v-if="showSubTitle">{{ yearMonthTitle }}</view>
      <view class="cub-calendar__weekdays" ref="weeksPanel">
        <view
          class="cub-calendar__weekday"
    ...
    <!-- content-->
    <view class="cub-calendar__content" ref="months" @scroll="mothsViewScroll">
      <view class="cub-calendar__panel" ref="monthsPanel">
        <view class="cub-calendar__body" ref="viewArea" :style="{ transform: `translateY(${translateY}px)` }">
          <view class="cub-calendar__month" v-for="(month, index) of compConthsData" :key="index">
            <view class="cub-calendar__month-title">{{ month.title }}</view>
            <view class="cub-calendar__days">
              <view class="cub-calendar__days-item" :class="type === 'range' ? 'cub-calendar__days-item--range' : ''">
                <template v-for="(day, i) of month.monthData" :key="i">
                  <view class="cub-calendar__day" :class="getClass(day, month, i)" @click="chooseDay(day, month)">
                    <!-- 日期显示slot -->
                    <view class="cub-calendar__day-value">
                      <slot name="day" :date="day.type == 'curr' ? day : ''">
                        {{ day.type == 'curr' ? day.day : '' }}
                      </slot>
                    </view>
                    <view class="cub-calendar__day-tips cub-calendar__day-tips--top" v-if="topInfo">
                      <slot name="top-info" :date="day.type == 'curr' ? day : ''"> </slot>
                    </view>
                    <view class="cub-calendar__day-tips cub-calendar__day-tips--bottom" v-if="bottomInfo">
                      <slot name="bottom-info" :date="day.type == 'curr' ? day : ''"> </slot>
                    </view>
                    <view class="cub-calendar__day-tips--curr" v-if="!bottomInfo && showToday && isCurrDay(day)">
                      {{ translate('today') }}
                    </view>
                    <view
                      class="cub-calendar__day-tip"
                      :class="{ 'cub-calendar__day-tips--top': rangeTip() }"
                      v-if="isStartTip(day, month)"
                    >
                      {{ startText || translate('start') }}
                    </view>
                    <view class="cub-calendar__day-tip" v-if="isEndTip(day, month)">{{
    ...
    <!-- footer-->
    <view class="cub-calendar__footer" v-if="poppable && !isAutoBackFill">
      <view class="cub-calendar__confirm" @click="confirm">{{ confirmText || translate('confirm') }}</view>
    </view>
  </view>
</template>
<script lang="ts">
  ...
  setup(props, { emit, slots }) {
    ...
    const isActive = (day: Day, month: MonthInfo) => {
      return (
        ...
        day.type == 'curr' &&
        getClass(day, month).includes('cub-calendar__day--active')
      );
    };
