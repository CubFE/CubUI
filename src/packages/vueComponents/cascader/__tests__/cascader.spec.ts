...
describe('Cascader', () => {
  ...
  it('visible', async () => {
    ...
    expect(wrapper.find('.cub-cascader__popup').isVisible()).toBe(false);

    await wrapper.setProps({
      visible: true
    });
    expect(wrapper.find('.cub-cascader__popup').isVisible()).toBe(true);
    expect(wrapper.html()).toMatchSnapshot();

    // TODO: 无法通过，每次更新props都重新执行了setup，原因待发现
    // // value为空时，会保留上次的选择记录
    // await wrapper.find('.cub-cascader-item').trigger('click');
    // expect(wrapper.html()).toMatchSnapshot();
    // expect(wrapper.findAll('.cub-tabs__titles-item__text').length).toBe(2);
    // await wrapper.setProps({
    //   visible: false
    // });
    // expect(wrapper.html()).toMatchSnapshot();
    ...
    await wrapper.findAll('.cub-cascader-pane')[2].find('.cub-cascader-item').trigger('click');
    // expect((wrapper.emitted('update:visible') as any)[0][0]).toBe(false);
    expect(
      wrapper
        .findAll('.cub-cascader-pane')[2]
        .find('.cub-cascader-item.active')
        .find('.cub-cascader-item__title')
        .html()
    ).toContain('鼓楼区');
  });
  ...
  it('modelValue', async () => {
    ...
    expect(wrapper.findAll('.cub-cascader-item[aria-checked="true"]').length).toBe(0);
    expect(wrapper.html()).toMatchSnapshot();
  ...
  it('select', async () => {
    ...
    // 模拟点击
    await wrapper.findAll('.cub-cascader-item')[2].trigger('click');
    ...
    await wrapper.findAll('.cub-cascader-pane')[1].find('.cub-cascader-item').trigger('click');
    ...
    await wrapper.findAll('.cub-cascader-item')[1].trigger('click');
    expect(wrapper.emitted().pathChange.length).toBe(3);
    expect(wrapper.emitted().change.length).toBe(1);
    expect(wrapper.html()).toMatchSnapshot();
  });
  ...
  it('select with lazy', async () => {
    ...
    await wrapper.findAll('.cub-cascader-item')[1].trigger('click');
    await later(10);
    await wrapper.findAll('.cub-cascader-item')[0].trigger('click');
    await later(60);
    let pathChange: any = wrapper.emitted().pathChange[0];
    ...
    await later();
    await wrapper.findAll('.cub-cascader-pane')[1].findAll('.cub-cascader-item')[0].trigger('click');
    await later(10);
    await wrapper.findAll('.cub-cascader-pane')[1].findAll('.cub-cascader-item')[1].trigger('click');
    await later(60);
    pathChange = wrapper.emitted().pathChange[1];
    ...
    await later();
    await wrapper.findAll('.cub-cascader-pane')[2].findAll('.cub-cascader-item')[0].trigger('click');
    pathChange = wrapper.emitted().pathChange[2];
  ...
  it('change tab', async () => {
    ...
    await wrapper.findAll('.cub-tabs__titles-item__text')[1].trigger('click');
    expect(wrapper.html()).toMatchSnapshot();

    await wrapper.findAll('.cub-tabs__titles-item__text')[0].trigger('click');
    expect(wrapper.html()).toMatchSnapshot();

    await wrapper.find('.cub-tabs__content .cub-cascader-item').trigger('click');
    expect(wrapper.findAll('.cub-tabs__titles-item__text').length).toBe(2);
    ...
    expect(wrapper.findAll('.cub-tabs__titles-item__text').length).toBe(3);
    expect(wrapper.findAll('.cub-tabs__titles-item__text')[2].text()).toBe('鼓楼区');
  });
});
