...
test('Collapse: should props active', async () => {
    ...
    return (
      <Collapse modelValue={['1', '2']}>
        <CollapseItem name="1">
          {{
            mTitle: () => '标题1',
            default: () => 'cubui是一套拥有轻量级的 Vue 组件库'
          }}
        </CollapseItem>
  ...
  const items = wrapper.findAll('.cub-collapse-item');
  expect(items.length).toEqual(2);
  await nextTick();
  expect(wrapper.html()).toMatchSnapshot();
  await sleep(1000);
  const icons = wrapper.findAll('.cub-collapse-item__title-icon--expanded');
...
test('Collapse: should props accordion', async () => {
  ...
  await sleep(1000);
  expect(wrapper.findAll('.cub-collapse-item__title-icon--expanded')).toHaveLength(1);
  const collapseWrapper = wrapper.findAll('.cub-collapse-item');
...
test('Collapse: should cub-collapse-item props ', async () => {
  const wrapper = mount(() => {
    return (
  ...
  await nextTick();
  const collapseItemTitle = wrapper.findAll('.cub-collapse-item__title-mtitle');
  ...
  const collapseWrapper = wrapper.findAll('.cub-collapse-item__title');
  expect(collapseWrapper[2].classes()).toContain('cub-collapse-item__title--disabled');
});
...
test('Collapse: v-model is undefined', async () => {
  ...
  // trigger change
  await nextTick();
  const collapseTitles = wrapper.findAll('.cub-collapse-item__title');
  ...
  await sleep(1000);
  const collapseWrappers = wrapper.findAll('.cub-collapse__item-wrapper');
