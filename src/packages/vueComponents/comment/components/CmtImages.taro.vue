<template>
  <view :class="`cub-comment-images cub-comment-images--${type}`">
    <!-- videos -->
    <view
      class="cub-comment-images__item cub-comment-images__item--video"
      v-for="(itV, index) in videos"
      :key="itV.id"
      @click="showImages('video', index)"
    >
      <img :src="itV.mainUrl" />
      <view class="cub-comment-images__play"></view>
    </view>
    <!-- images -->
    <template v-for="(itI, index) in images" :key="itI.id">
      <view
        class="cub-comment-images__item cub-comment-images__item--imgbox"
      ...
      >
        ...
        <view
          class="cub-comment-images__mask"
