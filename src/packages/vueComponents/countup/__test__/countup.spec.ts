...
test('basic usage props init-num end-num speed', async () => {
  const component = {
    template: `<cub-countup :init-num="initNum" :end-num="endNum" :speed="speed"></cub-countup>`,
    setup() {
      const data = reactive({
        initNum: 0,
        endNum: 4,
  ...
  const countupWrapper = wrapper.find('.cub-countup').element as Element;
  await nextTick();
...
test('basic usage props decimal places', async () => {
  const component = {
    template: `<cub-countup :init-num="200" :end-num="0" :speed="6.18" :to-fixed="toFixed"></cub-countup>`,
    setup() {
      const data = reactive({
        toFixed: 2
      });
...
test('digital scrolling during props', async () => {
  ...
  expect(wrapper.findAll('.cub-countup__number')).toHaveLength(1);

  const numberItemList = wrapper.findAll('.cub-countup__number-item');
  ...
  const itemSpan = wrapper.find('.cub-countup__number-item__span').element as HTMLElement;
...
test('custom img size props', async () => {
  ...
  await nextTick();
  await sleep(1000);
  const numberItemList = wrapper.findAll('.cub-countup__numberimg__item');
...
test('game machine props', async () => {
  ...
  await nextTick();
  // await sleep(2000);
  const machine = wrapper.findAll('.cub-countup__machine');
  expect(machine).toHaveLength(1);
  const machineItemList = wrapper.findAll('.cub-countup__machine-item');
