...
describe('function Dialog', () => {
  test('show dialog base info display ', async () => {
    ...
    expect(document.querySelector(`#dialog-${dialog.options.id} .cub-dialog`)).toBeTruthy();
    const textTitle = document.querySelector(
      `#dialog-${dialog.options.id} .cub-dialog .cub-dialog__header`
    ) as HTMLDivElement;
    expect(textTitle.innerHTML).toEqual('title');
    const content = document.querySelector(
      `#dialog-${dialog.options.id} .cub-dialog .cub-dialog__content view`
    ) as HTMLDivElement;
    expect(content.innerHTML).toEqual('content');
    const footer = document.querySelector(
      `#dialog-${dialog.options.id} .cub-dialog .cub-dialog__footer`
    ) as HTMLDivElement;
    expect(footer.children.length).toEqual(2);
    let cancelBtn = footer.querySelector(`#dialog-${dialog.options.id} .cub-dialog__footer-cancel`) as HTMLDivElement;
    cancelBtn.click();
  ...
  test('show dialog custom info display ', async () => {
    ...
    expect(footer.children.length).toEqual(2);
    let okBtn = footer.querySelector(`#dialog-${dialog.options.id} .cub-dialog__footer-ok`) as HTMLDivElement;
    okBtn.click();
  ...
  test('show dialog custom footer-direction ', async () => {
    ...
    expect(
      document.querySelector(`#dialog-${dialog.options.id} .cub-dialog .cub-dialog__footer.vertical`)
    ).toBeTruthy();
  });
  ...
  test('tips dialog', async () => {
    ...
    const footer = document.querySelector(
      `#dialog-${dialog.options.id} .cub-dialog .cub-dialog__footer-cancel`
    ) as HTMLDivElement;
    expect(footer).toBeNull();
  });
  ...
  test('dialog cancelText okText', async () => {
    ...
    const okText = document.querySelector(
      `#dialog-${dialog.options.id} .cub-dialog .cub-dialog__footer-ok`
    ) as HTMLDivElement;
  ...
  test('dialog teleport', async () => {
    ...
    const dialogDom = document.querySelector(`#dialog-${dialog.options.id} .cub-dialog`) as HTMLDivElement;
