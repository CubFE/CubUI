...
test('Dialog: basic props', async () => {
  ...
  nextTick(() => {
    const t = wrapper.find('.cub-dialog__header');
    expect(t.html()).includes('t1');
    const c = wrapper.find('.cub-dialog__content');
    expect(c.html()).includes('c1');
    const cancel = wrapper.find('.cub-dialog__footer-cancel');
    expect(cancel.html()).includes('cancelText');
    const ok = wrapper.find('.cub-dialog__footer-ok');
...
test('Dialog: closeOnClickOverlay', async () => {
  ...
  const overlay1 = wrapper1.find('.cub-overlay');
  overlay1.trigger('click');
  nextTick(() => {
    const popup1 = wrapper1.find('.cub-popup');
    expect(popup1.html().includes('display: none;')).toBeTruthy();
  });

  const overlay2 = wrapper2.find('.cub-overlay');
  overlay2.trigger('click');
  nextTick(() => {
    const popup2 = wrapper2.find('.cub-popup');
