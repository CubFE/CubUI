<template>
  <view class="cub-ecard">
    <view class="cub-ecard__title">{{ chooseText || translate('chooseText') }}</view>
    <view class="cub-ecard__list">
      <view
        v-for="(item, index) in dataList"
        :key="index"
        :class="['cub-ecard__list__item', currentIndex == index ? 'active' : '']"
        @click="handleClick(item, index)"
      >
        {{ item.price }}
      </view>
      <view :class="['cub-ecard__list__input', currentIndex == 'input' ? 'active' : '']" @click="inputClick">
        <view>{{ otherValueText || translate('otherValueText') }}</view>
        <view class="cub-ecard__list__input--con">
          <input
            class="cub-ecard__list__input--input"
            type="text"
            v-model="inputValue"
            @input="change"
      ...
      <view class="cub-ecard__list__step">
        <view>{{ suffix }}{{ money }}</view>
        <cub-input-number v-model="stepValue" :min="cardBuyMin" :max="cardBuyMax" @change="changeStep" />
      </view>
    </view>
  </view>
</template>
