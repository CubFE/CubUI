...
test('should render correct money', async () => {
  ...
  const item = wrapper.findAll('.cub-ecard__list__item');
...
test('input change when more than maxValue', async () => {
  ...
  const input: DOMWrapper<Element> = wrapper.find('.cub-ecard__list__input--input');
...
test('input change when less than maxValue', async () => {
  ...
  const input: DOMWrapper<Element> = wrapper.find('.cub-ecard__list__input');
  const add = wrapper.find('.cub-icon');
  input.trigger('click');
  add.trigger('click');
  await nextTick();
