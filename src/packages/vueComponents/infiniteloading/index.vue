<template>
  <view :class="classes" ref="scroller">
    <view class="cub-infinite__container">
      <slot></slot>
    </view>

    <view class="cub-infinite__bottom">
      <template v-if="isInfiniting">
        <view class="cub-infinite__bottom-box">
          <slot name="loading">
            <slot name="loading-icon"><Loading class="cub-icon-loading cub-infinite__bottom-box__img"></Loading></slot>
            <view class="cub-infinite__bottom-box__text">{{ loadTxt || translate('loading') }}</view>
          </slot>
        </view>
      </template>
      <template v-else-if="!hasMore">
        <slot name="finished">
          <view class="cub-infinite__bottom-tips">{{ loadMoreTxt || translate('loadMoreTxt') }}</view>
        </slot>
      </template>
    </view>
  </view>
