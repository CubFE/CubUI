...
test('should render clear icon when using clearable prop', async () => {
  ...
  wrapper.find('.cub-input-clear').trigger('click');
...
test('should clear when event clear', async () => {
  ...
  const input = wrapper.find('input');
  const clear: any = wrapper.find('.cub-input-clear-box');
...
test('should require', () => {
  ...
  const input = wrapper.find('.cub-input');
  expect(input.classes()).toContain('cub-input--required');
});
...
test('should render word limit correctly', () => {
  ...
  expect(wrapper.find('.cub-input-box').html()).toMatchSnapshot();
});
