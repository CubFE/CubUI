<template>
  <div class="demo">
    ...
    <cub-cell>
      <cub-input-number v-model="state.val1" />
    </cub-cell>
    <h2>{{ translate('step') }}</h2>
    <cub-cell>
      <cub-input-number v-model="state.val2" step="5" />
    </cub-cell>
    <h2>{{ translate('limit') }}</h2>
    <cub-cell>
      <cub-input-number v-model="state.val3" @overlimit="overlimit" min="10" max="20" />
    </cub-cell>
    <h2>{{ translate('disable') }}</h2>
    <cub-cell>
      <cub-input-number v-model="state.val4" disabled />
    </cub-cell>
    <h2>{{ translate('readonly') }}</h2>
    <cub-cell>
      <cub-input-number v-model="state.val5" readonly />
    </cub-cell>
    <h2>{{ translate('decimal') }}</h2>
    <cub-cell>
      <cub-input-number v-model="state.val6" step="0.1" decimal-places="1" readonly />
    </cub-cell>
    <h2>{{ translate('async') }}</h2>
    <cub-cell>
      <cub-input-number :model-value="state.val8" @change="onChange" />
    </cub-cell>
    <h2>{{ translate('size') }}</h2>
    <cub-cell>
      <cub-input-number v-model="state.val7" button-size="30" input-width="50" />
    </cub-cell>
    <h2>{{ translate('icon') }}</h2>
    <cub-cell>
      <cub-input-number v-model="state.val9">
        <template #left-icon>
          <Left />
        </template>
        <template #right-icon>
