<template>
  <view class="cub-menu-item" v-show="state.showWrapper" :style="style">
    <view
      v-show="state.showPopup"
      @click="handleClickOutside"
      class="cub-menu-item-placeholder-element"
    ...
    >
      <cub-scroll-view :scroll-y="true">
        <view class="cub-menu-item__content">
          <view
            v-for="(option, index) in options"
            :key="index"
            class="cub-menu-item__option"
            :class="[{ active: option.value === modelValue }]"
            :style="{ 'flex-basis': 100 / cols + '%' }"
            @click="onClick(option)"
          >
            <span
              class="cub-menu-item__span"
