<template>
  <view class="cub-navbar--placeholder" :style="rootStyle">
    <view :id="'navbarRef-' + refRandomId" :class="classes" :style="{ zIndex }" ref="navbarRef">
      <view class="cub-navbar__left" @click="handleLeft">
        <slot name="left-show" v-if="leftShow">
          <Left height="12px" color="#979797"></Left>
        </slot>
        <view v-if="leftText" class="cub-navbar__text">{{ leftText }}</view>
        <slot name="left"></slot>
      </view>
      <view class="cub-navbar__title">
      ...
      <view class="cub-navbar__right" @click="handleRight">
        <view v-if="desc" class="cub-navbar__text">{{ desc }}</view>
...
import { Left } from '@cubfe/icons-vue-taro';
