...
test('close event', async () => {
  ...
  const closeDom = wrapper.find('.cub-noticebar__page-righticon');
  ...
  await nextTick();
  const content = wrapper.find('.cub-noticebar__page-wrap-content');
...
test('vertical scroll', async () => {
  const wrapper = mount({
    components: {
      'cub-noticebar': Noticebar
    },
    template: `
      <cub-noticebar direction='vertical'  :list="horseLamp" ></cub-noticebar>
    `,
    setup() {
