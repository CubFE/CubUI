<template>
  <view :class="classes">
    <view
      ...
      class="cub-noticebar__page"
      :class="{
        'cub-noticebar__page--withicon': closeMode,
        'cub-noticebar__page--close': closeMode,
        'cub-noticebar__page--wrapable': wrapable
      }"
      :style="barStyle"
      @click="handleClick"
      v-if="direction == 'across'"
    >
      <view class="cub-noticebar__page-lefticon">
        <slot name="left-icon">
          <Notice v-if="leftIcon" size="16px"></Notice>
        </slot>
      </view>
      <view ref="wrap" :class="`cub-noticebar__page-wrap wrap${id}`">
        <view
          ref="content"
      ...
      <view v-if="closeMode || $slots['right-icon']" class="cub-noticebar__page-righticon" @click.stop="onClickIcon">
        <slot name="right-icon"> <CloseCircle /> </slot>
      </view>
    </view>

    <view
      class="cub-noticebar__vertical"
      v-if="scrollList.length > 0 && direction == 'vertical' && showNoticebar"
      :style="barStyle"
    >
      <template v-if="slots.default">
        <view class="cub-noticebar__vertical-list" :style="horseLampStyle">
        ...
        <view class="cub-noticebar-custom-item">
          <slot></slot>
        </view>
      </template>

      <template v-else>
        <ul class="cub-noticebar__vertical-list" :style="horseLampStyle">
          <li
            class="cub-noticebar__vertical-item"
            v-for="(item, index) in scrollList"
            :key="index"
      ...
      <view class="go" @click="handleClickIcon()">
        <slot name="right-icon">
          <CloseCircle v-if="closeMode" :color="color" size="11px" />
        </slot>
      </view>
    </view>
  </view>
...
import { Notice, CloseCircle } from '@cubfe/icons-vue-taro';
...
export default create({
  ...
  setup(props, { emit, slots }) {
    ...
    const wrapContentClass = computed(() => {
      return {
        'cub-noticebar__page-wrap-content': true,
        'cub-ellipsis': isEllipsis.value,
    ...
    const watchSlots = () => {
      setTimeout(() => {
        ...
        const ele = document.getElementsByClassName('cub-noticebar-custom-item')[0];
