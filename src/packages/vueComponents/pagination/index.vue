<template>
  <view class="cub-pagination">
    <view
      :class="['cub-pagination-prev', mode == 'multi' ? '' : 'simple-border', modelValue == 1 ? 'disabled' : '']"
    ...
    <view class="cub-pagination-contain" v-if="mode == 'multi'">
      <view
        v-for="(item, index) of pages"
        :key="index + 'pagination'"
        :class="['cub-pagination-item', item.active ? 'active' : '']"
    ...
    <view class="cub-pagination-contain" v-if="mode == 'simple'">
      <view class="cub-pagination-simple">{{ modelValue }}/{{ countRef }}</view>
    </view>
    <view
      :class="['cub-pagination-next', modelValue >= countRef ? 'disabled' : '']"
      @click="select(modelValue + 1, true)"
    >
      <slot name="next-text">
