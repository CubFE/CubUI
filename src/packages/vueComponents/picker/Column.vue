<template>
  <view class="cub-picker__list" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd">
    <view
      class="cub-picker-roller"
      ref="roller"
    ...
    >
      <template v-for="(item, index) in column" :key="item[fieldNames.value] ?? index">
        <!-- 3D 效果 -->
        <view
          class="cub-picker-roller-item"
          :class="{ 'cub-picker-roller-item-hidden': isHidden(index + 1) }"
        ...
        <!-- 平铺 -->
        <view
          class="cub-picker-roller-item-tile"
    ...
    <view class="cub-picker-roller-mask" :style="maskStyles"></view>
  </view>
</template>
<script lang="ts">
