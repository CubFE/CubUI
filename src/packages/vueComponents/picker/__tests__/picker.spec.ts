...
test('first render', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-picker__left').exists()).toBeTruthy();
...
test('simple list-data confirm & close event', async () => {
  ...
  await nextTick();
  wrapper.find('.cub-picker__left').trigger('click');
  wrapper.find('.cub-picker__right').trigger('click');
...
test('multiple columns render', async () => {
  ...
  await nextTick();
  const columnItems = wrapper.findAll('.cub-picker__columnitem');
