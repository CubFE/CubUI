<template>
  ...
  ></view>
  <view :class="['cub-popover', `cub-popover--${theme}`, `${customClass}`]" :style="popoverstyles" ref="popoverbox">
    <cub-popup
      :popClass="`cub-popover-content cub-popover-content--${location}`"
      :style="customStyle"
      v-model:visible="showPopup"
      position=""
      transition="cub-popover"
      :overlay="overlay"
      :duration="duration"
    ...
    >
      <view ref="popoverContentRef" :id="'popoverContentRef' + refRandomId" class="cub-popover-content-group">
        ...
        <view
          ...
          :class="[
            item.className,
            item.disabled && 'cub-popover-menu-disabled',
            'cub-popover-menu-item',
            'cub-popover-menu-taroitem'
          ]"
          @click.stop="chooseItem(item, index)"
        >
          <component v-if="item.icon" :is="renderIcon(item.icon)" class="cub-popover-item-img"></component>
          <view class="cub-popover-menu-item-name">{{ item.name }}</view>
        </view>
      </view>
    </cub-popup>

    <view class="cub-popover-content-bg" v-if="showPopup" @touchmove="clickAway" @click="clickAway"></view>
  </view>
  <!-- 用于计算大小 -->
  <!-- TODO -->
  <view :class="`cub-popover-content cub-popover-content-copy`">
    <view ref="popoverContentRefCopy" :id="'popoverContentRefCopy' + refRandomId" class="cub-popover-content-group">
      <view :class="popoverArrow" v-if="showArrow" :style="popoverArrowStyle"> </view>
      <slot name="content"></slot>
      <view
        v-for="(item, index) in list"
...
export default create({
  ...
  setup(props, { emit }) {
    ...
    const popoverArrow = computed(() => {
      const prefixCls = 'cub-popover-arrow';
