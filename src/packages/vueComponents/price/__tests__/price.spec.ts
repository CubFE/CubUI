...
test('Price: base price', () => {
  ...
  const price: any = wrapper.find('.cub-price');
...
test('Price: default needSymbol props', () => {
  const wrapper = mount(Price);
  const price: any = wrapper.find('.cub-price');
  expect(price.find('.cub-price--symbol').text()).toBe('¥');
});
test('Price: needSymbol props', () => {
  ...
  const price: any = wrapper.find('.cub-price');
  expect(price.find('.cub-price--symbol').exists()).toBe(false);
});
...
test('Price: size props', () => {
  ...
  const price: any = wrapper.find('.cub-price');
  expect(price.html()).toContain('cub-price--small');
});
