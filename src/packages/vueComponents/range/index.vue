<template>
  <view :class="containerClasses">
    <view class="cub-range-min" v-if="!hiddenRange">{{ +min }}</view>
    <view ref="root" :style="wrapperStyle" :class="classes" @click.stop="onClick">
      <view class="cub-range-mark" v-if="marksList.length > 0">
        <span v-for="marks in marksList" :key="marks" :class="markClassName(marks)" :style="marksStyle(marks)">
          {{ marks }}
          <span class="cub-range-tick" :style="tickStyle(marks)"></span>
        </span>
      </view>
      <view class="cub-range-bar" :style="barStyle">
        <template v-if="range">
          <view
            v-for="index of [0, 1]"
            :key="index"
            role="slider"
            :class="{
              'cub-range-button-wrapper-left': index == 0,
              'cub-range-button-wrapper-right': index == 1
            }"
          ...
          >
            ...
            <view class="cub-range-button" v-else :style="buttonStyle">
        ...
        <template v-else>
          <view
            role="slider"
            class="cub-range-button-wrapper"
            :tabindex="disabled ? -1 : 0"
            :aria-valuemin="+min"
    ...
    <view class="cub-range-max" v-if="!hiddenRange">{{ +max }}</view>
  </view>
</template>
<script lang="ts">
...
export default create({
  ...
  setup(props, { emit }) {
    ...
    const containerClasses = computed(() => {
      const prefixCls = 'cub-range-container';
      return {
        [prefixCls]: true,
        [`${prefixCls}-vertical`]: props.vertical
      };
    ...
    const markClassName = (mark: number) => {
      const classPrefix = 'cub-range-mark';
