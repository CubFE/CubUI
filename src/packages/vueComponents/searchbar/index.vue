<template>
  <view class="cub-searchbar" :style="searchbarStyle">
    <span class="cub-searchbar__search-label" v-if="label">{{ label }}</span>
    ...
    <view :class="[`cub-searchbar__search-input`, shape]" :style="{ ...inputSearchbarStyle, ...focusCss }">
      <view
        v-if="$slots.leftin"
        class="cub-searchbar__search-icon cub-searchbar__iptleft-search-icon"
        @click="leftIconClick"
      >
        <slot name="leftin"></slot>
      </view>
      <view :class="['cub-searchbar__input-inner', $slots.rightin && 'cub-searchbar__input-inner-absolute']">
        <form class="cub-searchbar__input-form" action="#" @submit.prevent="handleSubmit">
          <input
            ref="inputsearch"
