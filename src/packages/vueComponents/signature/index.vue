<template>
  <div :class="classes">
    <div class="cub-signature-inner" ref="wrap">
      <canvas ref="canvas" :height="canvasHeight" :width="canvasWidth" v-show="isCanvasSupported()"></canvas>
      <p class="cub-signature-unsopport" v-if="!isCanvasSupported()">{{ unSupportTpl || translate('unSupportTpl') }}</p>
    </div>

    <cub-button class="cub-signature-btn" type="default" @click="clear()">{{ translate('reSign') }}</cub-button>
    <cub-button class="cub-signature-btn" type="primary" @click="confirm()">{{ translate('confirm') }}</cub-button>
  </div>
</template>
<script lang="ts">
  ...
  props: {
    ...
    strokeStyle: {
      ...
      default: () => {
        ...
        if (clsName.indexOf('cub-theme-dark') == -1) {
          return '#000';
        } else {
          return '#fff';
        }
