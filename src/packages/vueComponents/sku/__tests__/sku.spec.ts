...
test('sku init render', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-sku-select').exists()).toBeTruthy();
});
...
test('sku select event', async () => {
  ...
  await nextTick();
  const skuItem = wrapper.findAll('.cub-sku-select-item-skus-sku');
...
test('do not sell', async () => {
  ...
  await nextTick();
  expect(wrapper.find('.cub-sku-operate-desc').exists()).toBeTruthy();
  expect(wrapper.find('.cub-sku-operate-desc').text()).toEqual('抱歉，此商品在所选区域暂无存货');
});
...
test('button event', async () => {
  ...
  await nextTick();
  const confirm = wrapper.find('.cub-sku-operate-btn-confirm');
