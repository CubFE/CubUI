<template>
  <view class="cub-sku-operate" v-if="btnOptions.length > 0">
    <view class="cub-sku-operate-desc" v-if="btnExtraText">{{ btnExtraText }}</view>

    <slot name="operate-btn"></slot>

    <view class="cub-sku-operate-btn" v-if="!getSlots('operate-btn')">
      <view
        :class="[`cub-sku-operate-btn-${btn}`, 'cub-sku-operate-btn-item']"
