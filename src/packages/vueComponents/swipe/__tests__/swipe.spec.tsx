...
test('Swipe: base swipe', () => {
  ...
  const swipe: DOMWrapper<Element> = wrapper.find('.cub-swipe');
...
test('Swipe: base swipe props disabled', async () => {
  ...
  await nextTick();
  const swipe1: DOMWrapper<Element> = wrapper.find('.cub-swipe__right');
  const swipe2: DOMWrapper<Element> = wrapper.find('.cub-button');
...
test('Swipe: base swipe Slots', async () => {
  ...
  await nextTick();
  const swipe: DOMWrapper<Element> = wrapper.find('.cub-swipe__left');
  const swipe2: DOMWrapper<Element> = wrapper.find('.cub-button');
  expect(swipe.exists()).toBe(true);
...
test('Swipe: base swipe content', async () => {
  ...
  await nextTick();
  const swipe2: DOMWrapper<Element> = wrapper.find('.cub-swipe__content');
  const swipe3: DOMWrapper<Element> = wrapper.find('.cub-cell');
  expect(swipe2.exists()).toBe(true);
...
test('Swipe: ref open & click close', async () => {
  ...
  await nextTick();
  expect(swipe.element.outerHTML).toContain('translate3d(-99');
  wrapper.find('.cub-swipe__content').trigger('click');
  await nextTick();
  expect(swipe.element.outerHTML).toContain('translate3d(0');
});
