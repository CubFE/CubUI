<template>
  <view
    class="cub-tabbar-item"
    :class="{ 'cub-tabbar-item__icon--unactive': !active }"
    :style="{
      color: active ? state.activeColor : state.unactiveColor
    }"
    @click="change()"
  >
    <cub-badge v-bind="$attrs">
      <view class="cub-tabbar-item_icon-box">
        <div class="cub-tabbar-item_icon-box_icon" v-if="isHaveSlot('icon')">
          <slot name="icon" :active="active"></slot>
        </div>
        <view v-if="icon && !isHaveSlot('icon')">
          <component :is="renderIcon(icon)" class="cub-popover-item-img"></component>
        </view>

        <view
          :class="[
            'cub-tabbar-item_icon-box_nav-word',
            { 'cub-tabbar-item_icon-box_big-word': !icon && !isHaveSlot('icon') }
          ]"
        >
          <slot>
