<template>
  <view :class="classes">
    <view class="cub-table__main" :class="{ 'cub-table__main--striped': striped }">
      <view class="cub-table__main__head">
        <view class="cub-table__main__head__tr">
          <span
            class="cub-table__main__head__tr__th"
          ...
          >
            {{ item.title }}
            <slot name="icon"></slot>
            <Under v-if="!$slots.icon && item.sorter" width="12px" height="12px"></Under>
          </span>
        </view>
      </view>
      <view class="cub-table__main__body">
        <view class="cub-table__main__body__tr" v-for="item in curData" :key="item">
          <span
            class="cub-table__main__body__tr__td"
    ...
    <view class="cub-table__nodata" v-if="!curData.length">
      <div class="cub-table__nodata" :class="{ 'cub-table__nodata--border': bordered }">
        <slot name="nodata"></slot>
        <div v-if="!$slots.nodata" class="cub-table__nodata__text"> {{ translate('noData') }} </div>
      </div>
    </view>
    <view class="cub-table__summary" v-if="summary">
      <span class="cub-table__summary__text" v-html="summary().value"></span>
    </view>
  </view>
</template>
...
import { Under } from '@cubfe/icons-vue';
...
export default create({
  ...
  setup(props: any, { emit }: any) {
    ...
    const cellClasses = (item: TableColumnProps) => {
      return {
        'cub-table__main__head__tr--border': props.bordered,
        [`cub-table__main__head__tr--align${item.align ? item.align : ''}`]: true
      };
    };
