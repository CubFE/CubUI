<template>
  <view :class="classes">
    <view v-if="readonly" class="cub-textarea__textarea cub-textarea__textarea__readonly">
      {{ modelValue }}
    </view>
    <textarea
      v-else
      ref="textareaRef"
      v-bind="$attrs"
      :class="['cub-textarea__textarea', env == 'ALIPAY' && 'cub-textarea__ali']"
      :style="styles"
      :rows="rows"
      :disabled="disabled"
      :readonly="readonly"
    ...
    />
    <view class="cub-textarea__limit" v-if="limitShow"> {{ modelValue ? modelValue.length : 0 }}/{{ maxLength }}</view>
    <view class="cub-textarea__cpoyText" :style="copyTxtStyle" v-if="autosize">{{ modelValue }}</view>
  </view>
</template>
...
export default create({
  ...
  setup(props, { emit }) {
    ...
    const copyHeight = () => {
      ...
      query.select('.cub-textarea__cpoyText').boundingClientRect();
      query.exec((res) => {
        if (res[0]) {
          if (props.modelValue == '') {
    ...
    const getRefHeight = () => {
      const query = Taro.createSelectorQuery();
      query.selectAll('.cub-textarea__textarea').boundingClientRect();
    ...
    const getRefWidth = () => {
      const query = Taro.createSelectorQuery();
      query.select('.cub-textarea__textarea').boundingClientRect();
      query.exec((res: any) => {
        if (res[0] && textareaRef.value) {
