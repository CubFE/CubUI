<template>
  <view :class="classes">
    <view class="cub-time-detail__detail cub-time-detail__detail--moring">
      <view class="cub-time-detail__detail__list">
...
export default create({
  ...
  setup: (props: any, context: any) => {
    ...
    const getClass = (item: string) => {
      ...
      if (find) {
        return {
          'cub-time-detail__detail__list__item': true,
          'cub-time-detail__detail__list__item--curr': find.list.filter((value: string) => value === item).length > 0
        };
      }
    };
