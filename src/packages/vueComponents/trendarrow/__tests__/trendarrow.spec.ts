...
import { Failure } from '@cubfe/icons-vue';
import { h } from 'vue';
...
test('should render rate when used ', async () => {
  ...
  expect(wrapper.find('.cub-trend-arrow-rate').text()).toBe('12.33%');
});
...
test('should render digits when used ', async () => {
  ...
  expect(wrapper.find('.cub-trend-arrow-rate').text()).toBe('12.3%');
});
...
test('should render showZero when used ', async () => {
  ...
  expect(wrapper.find('.cub-trend-arrow-rate').text()).toBe('0.00%');
});
test('should not render 0  when showZero is false ', async () => {
  ...
  expect(wrapper.find('.cub-trend-arrow-rate').text()).toBe('--');
});
test('should render left icon when arrowLeft', async () => {
  ...
  expect(wrapper.findAll('.cub-trend-arrow-icon-before').length).toBe(0);
  expect(wrapper.findAll('.cub-trend-arrow-icon-after').length).toBe(1);
});
test('should render sync text color when syncTextColor is true', async () => {
  ...
  const span = wrapper.find<HTMLElement>('.cub-trend-arrow-rate');
...
test('should render SVG icon when use down-icon slots', async () => {
  ...
  const icon = wrapper.find('.cub-icon');
