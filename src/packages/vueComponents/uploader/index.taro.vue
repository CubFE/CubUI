<template>
  <view class="cub-uploader">
    <view class="cub-uploader__slot" v-if="$slots.default">
      <slot></slot>
      <template v-if="Number(maximum) - fileList.length">
        <cub-button class="cub-uploader__input" @click="chooseImage" />
      </template>
    </view>

    <view class="cub-uploader__preview" :class="[listType]" v-for="(item, index) in fileList" :key="item.uid">
      <view class="cub-uploader__preview-img" v-if="listType == 'picture' && !$slots.default">
        <view class="cub-uploader__preview__progress" v-if="item.status != 'success'">
          <template v-if="item.status != 'ready'">
            <Failure color="#fff" v-if="item.status == 'error'" />
            <Loading name="loading" color="#fff" v-else />
          </template>
          <view class="cub-uploader__preview__progress__msg">{{ item.message }}</view>
        </view>
        ...
        <img
          class="cub-uploader__preview-img__c"
          :mode="mode"
          @click="fileItemClick(item)"
          v-if="(item?.type?.includes('image') || item?.type?.includes('video')) && item.url"
          :src="item.url"
        />
        <view v-else class="cub-uploader__preview-img__file">
          <view class="cub-uploader__preview-img__file__name" @click="fileItemClick(item)">
            <view class="file__name_tips">{{ item.name }}</view>
          </view>
        </view>
        <view class="tips">{{ item.name }}</view>
      </view>
      <view class="cub-uploader__preview-list" v-else-if="listType == 'list'">
        <view class="cub-uploader__preview-img__file__name" @click="fileItemClick(item)" :class="[item.status]">
          <Link class="cub-uploader__preview-img__file__link" />
          <view class="file__name_tips">{{ item.name }}</view>
          <Trash
            v-if="isDeletable"
            color="#808080"
            class="cub-uploader__preview-img__file__del"
    ...
    >
      ...
      <cub-button class="cub-uploader__input" :class="{ disabled }" @click="chooseImage" />
    </view>
  </view>
</template>
...
import { Photograph, Failure, Loading, Trash, Link } from '@cubfe/icons-vue-taro';
