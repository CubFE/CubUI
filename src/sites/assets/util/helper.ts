...
export const useThemeEditor = function () {
  ...
  onMounted(async () => {
    if (!store.init) {
      ...
      let customUrl = '';
      if (isJDT()) {
        customUrl = 'https://storage.360buyimg.com/cubui-static/source/variables-jdt.scss_source';
      } else if (isJDB()) {
        customUrl = 'https://storage.360buyimg.com/cubui-static/source/variables-jdb.scss_source';
      } else if (isJDDKH()) {
        customUrl = 'https://storage.360buyimg.com/cubui-static/source/variables-jddkh.scss_source';
      }
      if (customUrl) {
        loadScript('https://storage.360buyimg.com/cubui-static/cdn/sass.sync.min.js').then((res) => {
